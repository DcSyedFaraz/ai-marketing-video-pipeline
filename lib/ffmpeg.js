...
export async function concatMultipleVideos(videoPaths, outputPath, { width = 1080, height = 1920, xfadeDur = 0.4 } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Null audio source for clips that have no audio track
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (audioFlags[i]) {
        ...
        // Generate a silent clip exactly as long as the video
        const dur = (durations[i] || 5).toFixed(3);
        filters.push(
          `[${nullIdx}:a]atrim=duration=${dur},asetpts=PTS-STARTPTS,` +
          `aformat=sample_rates=44100:channel_layouts=stereo[sa${i}]`
        );
      }
    }
