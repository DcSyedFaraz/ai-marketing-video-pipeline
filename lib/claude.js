...
export async function generateQuickVideoScript({ audience, productPoint, duration, gameContext, showMobileScreen = true }) {
  ...
  const shotCount = dur <= 6 ? 2 : dur <= 10 ? 2 : 3;
  const shotGuidance = dur <= 6
    ? `SHOT STRUCTURE (2 shots for ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.55)}s): Opening — character in environment, emotional setup, building to speak.
  SHOT 2 (~${Math.round(dur * 0.45)}s): Close-up — character delivers dialogue directly to camera, reaction beat ends the clip.`
    : dur <= 10
    ? `SHOT STRUCTURE (2–3 shots for ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.4)}s): Wide/medium — character in environment, doing something real, emotional state visible.
  SHOT 2 (~${Math.round(dur * 0.35)}s): Medium — character turns/looks to camera, begins speaking.
  SHOT 3 (~${Math.round(dur * 0.25)}s): Close-up face — finishes line, reaction beat, expression holds.`
    : `SHOT STRUCTURE (3 shots for ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.3)}s): Establishing — character in environment, activity visible, emotional state set.
  SHOT 2 (~${Math.round(dur * 0.4)}s): Medium — character addresses camera, delivers main dialogue with full emotion.
  SHOT 3 (~${Math.round(dur * 0.3)}s): Close-up — reaction beat, final expression, scene breathes and ends naturally.`;
...
- Each shot has its own: location/framing, character action, and tone description.
- Shots should feel like different camera angles of the SAME moment — not a time jump or scene change.
...
═══ DEAD TIME RULES — ZERO TOLERANCE ═══
Every second must carry story value. These fillers are BANNED:
✗ Character gazing silently at screen for more than a glance
✗ Thumb moving on screen / tapping / swiping described in detail
✗ Character performing idle actions (rolling neck, reaching for something, adjusting clothing)
✗ Screen content described in detail (match timer, UI elements, score)
✗ Any action that doesn't directly serve the emotion or product point
Each shot must be purposeful — emotion, dialogue, or reaction. Nothing else.

═══ PHONE/SCREEN RULES ═══
${mobileScreenRule}
- When phone appears: character holds it up briefly, glances DOWN at screen once, then eyes snap BACK to camera. That is the full interaction. Nothing more.
- Never describe what is on the screen in detail. Never describe finger/thumb movements.
...
After the script shots, add a "NEGATIVE PROMPT:" line listing what to avoid for this specific scene. Always include: "multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode" plus 2–3 scene-specific additions based on what could glitch (e.g. "messy background" for a clean aesthetic, "children in frame" if not relevant, "outdoor setting" if indoors is required).

═══ FORMAT ═══
- Respond with valid JSON only. No markdown fences.
- script: plain text, 500–800 characters. Shot labels ("SHOT 1:", etc.) and "NEGATIVE PROMPT:" line are inside the script value.
- suggestedDuration: integer 4–${dur} based on dialogue length and shot count.
...
STRONG EXAMPLE of a 2-shot 8s script (adapt content, keep the format including negative prompt):
"SHOT 1 (medium, 4s): Softly blurred kitchen, warm morning light. A 34-year-old mom in a hoodie stands at the counter, coffee in hand, phone face-up beside her — eyes already bright before she even picks it up, like she's been thinking about this. SHOT 2 (close-up, 4s): She looks directly into the camera, voice hushed and fast, can't-keep-a-secret energy: \\"Three-minute PvP match in Blitz of Battle while my kid eats breakfast — I win more than I should.\\" She bites her lip to hide a grin. Warm candid light, intimate feel. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, messy kitchen, children in frame"
...
export async function generateQuickVideoStoryScript({ angle, audience, duration, gameContext, showMobileScreen = true }) {
  ...
  const shotGuidance = dur <= 6
    ? `SHOT STRUCTURE (2 shots for ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.55)}s): Opening — character in environment, emotional state visible, tension or curiosity established.
  SHOT 2 (~${Math.round(dur * 0.45)}s): Close-up — character delivers dialogue on camera, emotional payoff / reaction beat ends the clip.`
    : dur <= 10
    ? `SHOT STRUCTURE (2–3 shots for ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.4)}s): Wide/medium — character in their world, emotional state visible, angle's tension begins.
  SHOT 2 (~${Math.round(dur * 0.35)}s): Medium — character turns toward camera, engagement builds, story arc develops.
  SHOT 3 (~${Math.round(dur * 0.25)}s): Close-up face — delivers dialogue or reaction beat, emotional payoff lands.`
    : `SHOT STRUCTURE (3 shots for ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.3)}s): Establishing — character in environment, angle's emotional tension introduced.
  SHOT 2 (~${Math.round(dur * 0.4)}s): Medium — character addresses camera, delivers main dialogue with full emotion, story arc peaks.
  SHOT 3 (~${Math.round(dur * 0.3)}s): Close-up — reaction beat, final expression, emotional payoff that resolves the angle's tension.`;
...
- The narrative arc must build across shots — not the same moment repeated at different angles.
- Shots should feel like a real ad cut together — each one advances the story.
...
✗ Character performing idle actions (rolling neck, reaching for something, adjusting clothing)
✗ Screen content described in detail (match timer, UI elements, score)
✗ Any action that doesn't directly serve the angle's emotional territory
Each shot must be purposeful — emotion, dialogue, or reaction. Nothing else.
...
═══ NEGATIVE PROMPT HEADING ═══
After the script, add a separate "NEGATIVE PROMPT:" line listing what to avoid for this specific scene. This heading is part of the output and helps Seedance suppress glitches. Always include: "multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode" plus 2–3 scene-specific additions.

═══ FORMAT ═══
- Respond with valid JSON only. No markdown, no explanation outside JSON.
- script: plain text 500–800 characters. Shot labels ("SHOT 1:", etc.) and "NEGATIVE PROMPT:" line are inside the script value.
- suggestedDuration: integer 4–${dur} based on scene pacing, dialogue length, and shot count.
...
STRONG EXAMPLE of a 3-shot 12s story script (adapt content, keep the format and negative prompt):
"SHOT 1 (wide, 4s): Dimly lit bedroom, blue glow from phone. A 28-year-old man sits on the edge of his bed — shoulders hunched, clearly losing, jaw tight. He taps once, stares at the screen, a small defeated exhale. SHOT 2 (medium, 5s): He sits up straighter — something shifts in his eyes. He holds the phone at an angle and glances down once, then looks straight into the camera, voice low and intense: \\"One more match. They're not ready for what I learned.\\" SHOT 3 (close-up, 3s): A slow exhale through the nose — he almost smiles. His eyes say he's already won. The glow of the screen catches his face. Quiet, earned satisfaction. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, surrender screen, game-over UI"
