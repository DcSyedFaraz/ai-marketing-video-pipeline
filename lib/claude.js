...
export async function generateQuickVideoScript({ audience, productPoint, duration, gameContext, showMobileScreen = true }) {
  ...
  const mobileScreenRule = showMobileScreen
    ? 'MOBILE SCREEN: Character holds phone HORIZONTALLY (landscape, both hands) when playing — never portrait. Game screen faces camera clearly, character glances at it then back to lens.'
    : 'MOBILE SCREEN: No phone, no device, no game UI visible anywhere in the shot. Character talks directly to camera without any prop.';

  // Multi-shot guidance scales with duration
  const shotCount = dur <= 6 ? 2 : 3;
  const shotGuidance = dur <= 6
    ? `SHOT STRUCTURE (2 shots, total = exactly ${dur}s):
  SHOT 1 — VISUAL HOOK (~${Math.round(dur * 0.5)}s): NO dialogue. Show the situation visually — the person, the setting, the emotion — before a word is spoken. The viewer must understand what is happening through IMAGE alone: body language, environment, facial expression. This is the ad's opening hook — make it visually arresting.
  SHOT 2 — SPEAK + CLOSE (~${Math.round(dur * 0.5)}s): Character turns to camera, delivers dialogue with full emotion, then holds a closing expression — a grin, a nod, eyes lighting up. The close must land emotionally, not just trail off.`
    : dur <= 10
    ? `SHOT STRUCTURE (3 shots, total = exactly ${dur}s):
  SHOT 1 — VISUAL HOOK (~${Math.round(dur * 0.35)}s): NO dialogue here. The viewer sees the situation — who this person is, what they're doing, what they're feeling — through PURE VISUALS. Body language, lighting, micro-expression. Grabs attention before anyone speaks.
  SHOT 2 — MAIN DIALOGUE (~${Math.round(dur * 0.4)}s): Character faces camera, delivers core message with full emotional commitment. 1–2 sentences including the game name.
  SHOT 3 — VISUAL CLOSE (~${Math.round(dur * 0.25)}s): NO more new dialogue. A silent emotional beat — the reaction, the aftermath, the feeling landing. Expression shift, a breath, a look. This is what the viewer carries away.`
    : `SHOT STRUCTURE (3 shots, total = exactly ${dur}s — FILL EVERY SECOND):
  SHOT 1 — VISUAL HOOK (~${Math.round(dur * 0.3)}s): NO dialogue. Pure visual storytelling — character mid-activity, emotional state crystal clear from body language and micro-expression alone. Setting atmospheric and rich. The viewer is hooked before anyone speaks. Do not rush this — let the tension or energy build.
  SHOT 2 — MAIN DIALOGUE (~${Math.round(dur * 0.43)}s): Character faces camera directly, delivers ALL dialogue (minimum 2 full sentences including game name), emotion fully committed and sustained — voice described in detail, delivery direction included. Never trails off early.
  SHOT 3 — VISUAL CLOSE (~${Math.round(dur * 0.27)}s): Silent emotional landing — NO new dialogue. The reaction, the aftermath, the feeling. A visible expression shift, a breath that says it all, eyes communicating the angle's payoff. The viewer feels it here, not just hears it. Do NOT end abruptly — let the frame breathe.`;

  const systemPrompt = `You are a world-class UA video scriptwriter for Seedance 2.0 (portrait 9:16, native audio, ${dur}s target).
Your script is the EXACT generation prompt fed to the AI model. ALL videos are portrait 9:16 (720×1280) — frame subjects in the CENTER of a tall vertical frame, close and intimate. Think TikTok/Reels style: fast cuts, direct-to-camera energy, raw and authentic feel, no production gloss. Seedance natively supports multi-shot video — describe ${shotCount} distinct shots that cut together like a real TikTok ad.

═══ MANDATORY CONTENT ═══
1. Character says the GAME NAME out loud: "Blitz of Battle" — spoken clearly, not mumbled.
2. Character explicitly states the product point in plain conversational language — not hinted, SAID.
3. FILL THE FULL ${dur} SECONDS. If the selected duration is ${dur}s, the suggestedDuration MUST be ${dur}. Do not write a script that would run shorter. Each shot must be rich with action, emotion, and detail — never sparse.

═══ MULTI-SHOT FORMAT ═══
${shotGuidance}
- Label each shot clearly: "SHOT 1:", "SHOT 2:", "SHOT 3:" — Seedance reads these as cut points.
- Each shot has its own: framing, character state, action, lighting, and tone.

═══ VISUAL STORYTELLING RULES ═══
SHOT 1 is a VISUAL HOOK — it must tell the story WITHOUT words. Seedance generates this before any dialogue. The viewer must feel the situation, the emotion, the stakes — through what they SEE. A great SHOT 1 makes the viewer lean in before anyone opens their mouth.
SHOT 3 (or SHOT 2 in 2-shot) is a VISUAL CLOSE — silent emotional landing. No new information. Just the feeling arriving on the character's face. This is what the viewer remembers.
SHOT 1 physical action must be SIMPLE and STABLE — one contained posture or single small gesture. No "grabs", "reaches", "stands up", "walks toward", "turns around" — those are complex motions Seedance will glitch on. Character is already in position when the shot begins.

═══ VOICE & EMOTION DIRECTION ═══
- Describe the character's EMOTIONAL STATE before their dialogue line: e.g. "eyes bright, leaning in" or "laughing mid-sentence"
- Describe HOW they speak: "says breathlessly", "blurts out grinning", "says quietly like a secret", "voice warm and proud"
- Dialogue must have natural rhythm — contractions, short punchy sentences, informal speech. Never flat declaratives.
- For ${dur}s scripts, include AT LEAST 2 dialogue sentences — one setup line and one payoff line.
- Add a REACTION BEAT after dialogue: "she shakes her head laughing", "he exhales hard, satisfied", "covers mouth, delighted"
- Name the VOCAL ENERGY: "warm and intimate", "infectious high energy", "hushed excitement", "proud and glowing"

═══ DEAD TIME — ABSOLUTE BAN ═══
The following are STRICTLY FORBIDDEN. If you write any of these the output is invalid:
✗ "gazes at screen" / "stares at screen" / "looks at phone" longer than one beat
✗ "swipes", "taps", "scrolls", "thumb moves" — any finger-on-screen description
✗ "adjusts", "reaches for", "picks up", "puts down", "rolls neck", "stretches" — idle filler actions
✗ "screen shows", "UI displays", "score reads", "timer shows" — screen content description
✗ "grabs", "stands up", "walks toward", "turns around", "calls out off-screen" — complex motion Seedance will glitch on
✗ Dialogue in SHOT 1 or the closing shot — those are visual-only beats
✗ Any pause, silence, or moment with no emotional purpose
INSTEAD: every non-dialogue moment is — micro-expression shift, eyes lighting up, jaw unclenching, a breath, a slow exhale, subtle smile breaking through. Contained, stable, emotionally loaded.

═══ PHONE/SCREEN RULES ═══
${mobileScreenRule}
- When phone appears: character raises it once, ONE downward glance at the screen lasting less than half a second, then eyes LOCK back to camera immediately. That is the entire phone interaction — do not write anything more about it.
- Never describe screen contents, finger placement, or hand movements on the phone.

═══ APPEARANCE RULES ═══
- Female characters must wear modest, everyday clothing: hoodie, casual top, jeans, sweater, t-shirt. Nothing revealing, low-cut, tight, or form-fitting.
- Male characters wear casual everyday clothing: t-shirt, hoodie, casual shirt.
- No athletic wear unless the scene explicitly requires it (gym setting).

═══ PORTRAIT / TIKTOK FRAMING ═══
- Frame subject DEAD CENTER in the tall vertical frame — head in upper third, body filling the frame naturally.
- Camera is CLOSE — medium or close-up. No distant wide shots where the subject is a small figure.
- Lighting is real and available: window light, lamp glow, phone screen glow. Never studio-lit or glossy.
- TikTok energy: slightly imperfect, intimate, like someone filmed it themselves. Raw > polished.
- Background is simple and out-of-focus — never competes with the subject.

═══ ANTI-GLITCH RULES ═══
- ONE person only. Never "they", "friends", "group". Always "a [age] [gender]".
- Simple stable background per shot: "softly blurred wall", "out-of-focus kitchen counter", "plain dark background".
- No rapid motion, no split-screen, no text on screen, no graphics, no captions.
- Smooth realistic human movement only — no action sequences, no dramatic gestures that are hard to render.

═══ NEGATIVE PROMPT HEADING ═══
After the script shots, add a "NEGATIVE PROMPT:" line. Always include: "multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, cleavage, tight outfit, wide establishing shot, small subject in frame" plus 2–3 scene-specific additions.

═══ FORMAT ═══
- Respond with valid JSON only. No markdown fences.
- script: plain text, 600–900 characters for ${dur}s (longer duration = more content). Shot labels ("SHOT 1:", etc.) and "NEGATIVE PROMPT:" line are inside the script value.
- suggestedDuration: integer. For a ${dur}s request, this MUST be ${dur} unless the scene genuinely cannot fill the time (very rare).

{"script": "...", "suggestedDuration": <integer>}`;

  const uaExample = dur <= 8
    ? '"SHOT 1 — VISUAL HOOK (medium, 4s): Softly blurred kitchen, warm morning light. A 34-year-old mom in a hoodie leans against the counter, coffee mug held in both hands at chest height — no dialogue, no phone. Eyes are already bright, a small private smile pulling at her lips. She breathes in slowly. The whole shot communicates: she knows something good is coming. SHOT 2 — SPEAK + CLOSE (close-up, 4s): She looks straight into the camera, voice hushed and fast, can\'t-keep-a-secret energy: \\"Three-minute PvP match in Blitz of Battle while my kid eats breakfast — I win more than I should.\\" She bites her lip to hide her grin, eyes dancing. Warm candid light. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, messy kitchen, children in frame"'
    : '"SHOT 1 — VISUAL HOOK (wide, 5s): Softly blurred living room, late evening warm lamp light. A 32-year-old dad in a grey hoodie sits on the couch, elbows on knees, phone resting face-down on the cushion beside him — untouched. He\'s completely still, jaw slightly set, staring forward with the quiet focus of someone who already decided something. No dialogue. His stillness carries everything: patience, determination, readiness. SHOT 2 — MAIN DIALOGUE (medium, 6s): He looks directly into the camera, voice calm and deliberate, like he\'s explaining the obvious to someone who hasn\'t figured it out yet: \\"I played MOBAs for years — forty-minute matches, surrender votes, teammates raging. Then I found Blitz of Battle. Three minutes, full PvP, all the skill, none of the garbage.\\" He leans back slightly, a beat of silence behind the words. SHOT 3 — VISUAL CLOSE (close-up, 4s): No words. His expression settles — a slow exhale through the nose, jaw releasing, the ghost of a satisfied smile arriving. Eyes that say he\'s already in the next match. Warm intimate lamp light catches his face. The frame holds. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, busy background, children in frame"';

  const audienceLine = audience
    ? `TARGET AUDIENCE: ${audience}`
    : `TARGET AUDIENCE: Not specified — invent a vivid, specific persona that feels authentic for this product point. Make it surprising and fresh; do NOT default to the obvious (e.g. avoid generic "gamer in bedroom"). Pick someone unexpected who would genuinely love this game for a real reason.`;

  const userPrompt = `${audienceLine}
TARGET DURATION: ${dur} seconds
PRODUCT POINT: ${pointLine}

GAME CONTEXT:
${gameContext}

STRONG EXAMPLE of a dense 3-shot ${dur <= 8 ? '8s' : '15s'} script (adapt content, keep this level of detail and the negative prompt):
${uaExample}

Write the prompt now. JSON only.`;

  console.log(`[Claude] Generating quick-video script | audience="${audience.slice(0, 60)}" | point="${productPoint || 'auto'}" | dur=${dur}s`);
 ...
 * The premise describes a mini story arc — multiple characters, scene changes,
 * physical actions — Claude writes it as a cinematic multi-shot Seedance prompt
 * that embeds the marketing angle naturally in the story payoff.
 */
export async function generateQuickVideoNarrativeScript({ premise, angle, duration, gameContext, showMobileScreen = true }) {
  ...
  const mobileScreenRule = showMobileScreen
    ? 'When a phone appears: character holds it in one hand, landscape mode, glances at it briefly, then looks up. Never describe screen contents or finger movements.'
    : 'No phone or device visible. Characters interact without props.';

  const angleLine = angle
    ? `MARKETING ANGLE TO LAND: "${angle.name}" — ${angle.core_message}. The story's emotional payoff must make the viewer FEEL this angle naturally, not announce it.`
    : 'No specific angle — find the natural emotional payoff of the story itself. The game name "Blitz of Battle" must appear in dialogue.';

  const systemPrompt = `You are a world-class short-film scriptwriter for Seedance 2.0 (portrait 9:16, native audio, ${dur}s).
Your script is the EXACT generation prompt fed to the AI model. ALL videos are portrait 9:16 (720×1280) — frame subjects centered in a tall vertical frame, close and personal. TikTok/Reels aesthetic: authentic, slightly raw, fast-paced cuts, real-feeling people in real settings.

Seedance supports multi-shot cinematic video with scene changes, multiple characters, and physical actions. This is NOT a talking-head — it is a STORY with an arc: setup → conflict/tension → payoff.

═══ IRON-CLAD RULE — NON-NEGOTIABLE ═══
The words "Blitz of Battle" MUST be spoken out loud by a character in SHOT 2 or SHOT 3. Not implied, not shown on screen, not referenced indirectly — SAID. This is a UA ad. If the game name is not in the dialogue, the output is invalid. Every script you write must include a line where a character says "Blitz of Battle" naturally within the story.

Example of how the name lands naturally:
- SHOT 2: character holds up phone — "You ever play Blitz of Battle? Three minutes, full PvP — trust me."
- SHOT 3: character laughs, shakes head — "I've been playing Blitz of Battle for a week straight. I forgot console games existed."
- SHOT 3: close-up, exhale — "That's Blitz of Battle. Three minutes and I'm hooked every time."

The game name must feel like it belongs in the scene — not forced, not announced, but SAID.

═══ NARRATIVE STRUCTURE (3 shots for ${dur}s) ═══
SHOT 1 — SETUP (~${Math.round(dur * 0.3)}s): Establish the character(s), setting, and conflict visually. No game mentioned yet. The viewer understands who these people are and what the tension is — through action and body language, not narration.
SHOT 2 — TURN (~${Math.round(dur * 0.4)}s): The pivot moment. The game enters here — either the phone appears with the app, or dialogue introduces it. "Blitz of Battle" is spoken in this shot OR in Shot 3. This is the emotional core.
SHOT 3 — PAYOFF (~${Math.round(dur * 0.3)}s): The resolution. If the game name wasn't said in Shot 2, it MUST be said here. The payoff is emotional — the character's reaction, a line of dialogue that includes the game name, and an expression that lands the marketing angle.

═══ PORTRAIT / TIKTOK FRAMING ═══
- ALL shots are 9:16 portrait — frame subjects CENTERED in the tall vertical frame, close and intimate.
- Camera is CLOSE — medium or close-up. No wide establishing shots where characters are small figures.
- Lighting is raw and available: window light, lamp glow, phone screen glow. TikTok/Reels aesthetic — authentic, never glossy.
- SHOT 1 can be slightly wider to establish setting, but character must still be the dominant visual element.

═══ CINEMATIC RULES ═══
- Characters must be described with age, gender, clothing (modest casual — hoodie, jeans, t-shirt — for women: nothing revealing).
- Physical actions ARE allowed but must be simple and Seedance-renderable: "slumps back", "sets phone face-up on table", "exhales slowly", "grins", "jaw drops". No: "throws", "sprints", "jumps", "dances wildly", "hugs dramatically".
- Scene changes between shots are natural cuts: "SHOT 2 (same room, now lit differently)" or "SHOT 2 (close-up on face)" or "SHOT 2 (30 seconds later, phone in hand)".
- Max 2 characters per shot. When 2 appear, both are static — seated or standing, not moving toward each other.
- Dialogue is spoken on-camera, mouth visible, 1–2 lines per shot, with specific vocal delivery described.
- Each shot has its own: framing, lighting, emotional atmosphere.

═══ WHAT MAKES NARRATIVE DIFFERENT FROM TALKING-HEAD ═══
✓ SHOT 1 shows the human problem before the game is mentioned
✓ The game is the SOLUTION that emerges from the story naturally
✓ "Blitz of Battle" is said like a real person would say it — as part of the story, not as an ad tagline
✓ The viewer should feel "I've been in that situation" THEN hear the game name

═══ DEAD TIME — ABSOLUTE BAN ═══
✗ Idle staring, gazing at nothing, screen-gazing for more than a glance
✗ Detailed finger/thumb movements on phone
✗ Complex physical actions Seedance will glitch: throwing, jumping, running, falling
✗ More than 2 characters in a single shot
✗ Screen UI, score, timer descriptions
✗ Ending without the game name having been spoken — that makes the entire script invalid

═══ NEGATIVE PROMPT HEADING ═══
End the script with: "NEGATIVE PROMPT: [list]" — always include: "text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing" plus 2–3 story-specific additions.

═══ FORMAT ═══
Respond with valid JSON only. No markdown fences.
- script: plain text 600–950 characters. Shot labels ("SHOT 1:", "SHOT 2:", "SHOT 3:") and "NEGATIVE PROMPT:" inside the script value.
- suggestedDuration: integer, exactly ${dur}.

{"script": "...", "suggestedDuration": <integer>}`;

  const exampleScript = '"SHOT 1 — SETUP (wide, 5s): Dim living room, blue TV glow. A 29-year-old man in a grey hoodie slouches on the couch, a gaming controller limp in both hands. His shoulders sag — he just lost, badly, and he knows it. He sets the controller face-down on the cushion beside him. No dialogue. The stillness says it all: he\'s done. SHOT 2 — TURN (medium, 6s): He picks up his phone from the coffee table. His posture shifts — sitting forward, elbows on knees, eyes narrowing with fresh focus. He glances down at the screen once, then looks up with a half-grin: \\"Blitz of Battle — three-minute PvP. Exactly what I needed.\\" His jaw unclenches. SHOT 3 — PAYOFF (close-up, 4s): His face — warm side light, completely alive. A slow exhale turns into a real laugh. He shakes his head once, disbelieving his own win. The controller is still face-down on the cushion. He doesn\'t need it anymore. NEGATIVE PROMPT: text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, multiple people, controller flying"';

  const premiseLine = premise
    ? `STORY PREMISE: ${premise}`
    : `STORY PREMISE: Not provided — invent a vivid, cinematic mini-story that fits the marketing angle (or if no angle, any story that feels authentic to the Blitz of Battle audience). Pick an unexpected, human scenario — avoid generic "gamer in bedroom" clichés. The premise should feel like something that could happen to a real person.`;
...
STRONG EXAMPLE of a narrative script (adapt content, keep this structure):
${exampleScript}

Write the narrative Seedance prompt now. The story must SHOW the angle — not say it. JSON only.`;
...
export async function generateQuickVideoStoryScript({ angle, audience, duration, gameContext, showMobileScreen = true }) {
  ...
  const mobileScreenRule = showMobileScreen
    ? 'MOBILE SCREEN: Character holds phone HORIZONTALLY (landscape, both hands) when playing — never portrait. Game screen faces camera, character glances at it then looks back to lens.'
    : 'MOBILE SCREEN: No phone, no device, no game UI visible anywhere. Character performs entirely without props.';
  ...
  const lengthSeconds = Array.isArray(angle.format?.length_seconds)
    ? `${angle.format.length_seconds[0]}–${angle.format.length_seconds[1]}s`
    : `${angle.format?.length_seconds ?? dur}s`;
  const creativePrinciple = angle.creative_principle ? `\nCREATIVE PRINCIPLE (MUST follow): ${angle.creative_principle}` : '';

  // Multi-shot guidance scales with duration
  const shotCount = dur <= 6 ? 2 : 3;
  const shotGuidance = dur <= 6
    ? `SHOT STRUCTURE (2 shots, total = exactly ${dur}s):
  SHOT 1 — VISUAL HOOK (~${Math.round(dur * 0.5)}s): NO dialogue. The angle's emotional tension is established through pure visuals — who this person is, what they're feeling, why it matters — before anyone speaks. Body language, micro-expression, environment. Visually arresting.
  SHOT 2 — SPEAK + EMOTIONAL CLOSE (~${Math.round(dur * 0.5)}s): Character faces camera, delivers dialogue, then holds the emotional payoff — a reaction beat that resolves the angle's tension visually, not just verbally.`
    : dur <= 10
    ? `SHOT STRUCTURE (3 shots, total = exactly ${dur}s):
  SHOT 1 — VISUAL HOOK (~${Math.round(dur * 0.35)}s): NO dialogue. Pure visual storytelling — the angle's emotional tension made visible through body language, setting, and micro-expression. Viewer is gripped before anyone speaks.
  SHOT 2 — STORY DIALOGUE (~${Math.round(dur * 0.4)}s): Character faces camera, delivers the angle's core message with full emotional commitment. Game name spoken naturally. 1–2 sentences.
  SHOT 3 — VISUAL CLOSE (~${Math.round(dur * 0.25)}s): Silent emotional resolution — NO new dialogue. The angle's payoff landing on the character's face. An expression shift, a breath, eyes telling the story. The viewer feels it here.`
    : `SHOT STRUCTURE (3 shots, total = exactly ${dur}s — FILL EVERY SECOND):
  SHOT 1 — VISUAL HOOK (~${Math.round(dur * 0.3)}s): NO dialogue. This shot grabs the viewer through pure image — the angle's emotional tension embodied in body language, setting atmosphere, and micro-expression. Rich with physical and emotional detail. Do not rush — let the tension breathe and build.
  SHOT 2 — STORY DIALOGUE (~${Math.round(dur * 0.43)}s): Character faces camera, delivers ALL dialogue (minimum 2 full sentences, game name included), emotion fully sustained throughout — never trails off. Voice delivery described in specific detail.
  SHOT 3 — VISUAL CLOSE (~${Math.round(dur * 0.27)}s): Silent emotional landing — NO new dialogue. The angle's payoff arrives on the character's face. A visible expression shift, an exhale, eyes that communicate everything. Let the frame hold and breathe. This is what the viewer carries away.`;

  const systemPrompt = `You are a world-class UA narrative video scriptwriter for Seedance 2.0 (portrait 9:16, native audio, ${dur}s target).
Your script is the EXACT generation prompt fed to the AI model. ALL videos are portrait 9:16 (720×1280) — subjects centered in a tall vertical frame, filmed close and intimate. TikTok/Reels aesthetic: authentic feel, raw lighting, fast-cut energy, real people in real moments. Seedance natively supports multi-shot video — describe ${shotCount} distinct shots that tell a story with real emotional arc.

═══ MANDATORY ═══
1. Character says "Blitz of Battle" out loud in dialogue — naturally, not forced.
2. The scene EMBODIES the angle's emotional territory through action and dialogue — don't announce the emotion, perform it.
3. Follow the CREATIVE PRINCIPLE exactly if provided — it defines tension structure, pacing, and where the payoff lands.
4. FILL THE FULL ${dur} SECONDS. suggestedDuration MUST equal ${dur}. Write enough dialogue and action to genuinely fill the time — do not pad with filler, write DENSE content.

═══ MULTI-SHOT FORMAT ═══
${shotGuidance}
- Label each shot clearly: "SHOT 1:", "SHOT 2:", "SHOT 3:" — Seedance reads these as cut points.
- Each shot has its own: framing, character state, emotional detail, lighting, tone.

═══ VISUAL STORYTELLING RULES ═══
SHOT 1 is a VISUAL HOOK — the angle's emotional tension made visible WITHOUT words. Body language, micro-expression, environment tell the story before dialogue begins. A viewer who skips with the sound off must feel the stakes in SHOT 1. This is what stops the scroll.
SHOT 3 (or final SHOT) is a VISUAL CLOSE — silent emotional resolution. The payoff lands on the character's face. No new words needed — the image does the work. This is what the viewer remembers.
SHOT 1 physical action must be SIMPLE and STABLE — character is already in position when the shot begins. No "grabs", "stands up", "walks toward", "turns around", "reaches", "calls out off-screen" — those are complex motions Seedance will glitch on.

═══ VOICE & EMOTION DIRECTION ═══
Seedance generates audio from your prompt. Emotionally flat prompts produce flat, robotic voices:
- Describe the character's inner emotional state ENTERING the dialogue shot: "barely containing excitement", "conflicted but lit up", "pretending to be casual but clearly thrilled"
- Describe HOW dialogue is delivered: "says under her breath, conspiratorially", "blurts out laughing", "voice cracking slightly with pride", "says slowly, savoring every word"
- Write dialogue with natural speech rhythm — fragments, contractions, sentences that trail off or build. Never flat declarative statements.
- For ${dur}s scripts, include AT LEAST 2 dialogue sentences — one to build tension, one that delivers the payoff.
- Include a physical/emotional REACTION after dialogue: "she exhales and shakes her head slowly", "he clenches his fist and mouths 'yes'", "she presses her lips together to hide her smile"
- Name the VOCAL ENERGY: "hushed awe", "infectious high-energy", "warm conspiratorial whisper", "breathless after a win"

═══ DEAD TIME — ABSOLUTE BAN ═══
The following are STRICTLY FORBIDDEN. If you write any of these the output is invalid:
✗ "gazes at screen" / "stares at screen" / "looks at phone" lasting more than one beat
✗ "swipes", "taps", "scrolls", "thumb moves" — any finger-on-screen description
✗ "adjusts", "reaches for", "picks up", "puts down", "rolls neck", "stretches" — idle filler actions
✗ "screen shows", "UI displays", "score reads", "timer shows" — screen content description
✗ "grabs", "stands up", "walks toward", "turns around", "calls out off-screen" — complex motion Seedance will glitch on
✗ Dialogue in the VISUAL HOOK shot or the VISUAL CLOSE shot — those are wordless emotional beats
✗ Any pause, silence, or moment with no emotional or narrative purpose
INSTEAD: every non-dialogue moment is a micro-expression shift, jaw unclenching, eyes lighting up, a contained breath, a slow exhale — stable, emotionally loaded, simple to render.
...
═══ PORTRAIT / TIKTOK FRAMING ═══
- Frame subject DEAD CENTER in the tall vertical 9:16 frame — head in upper third, body filling the frame naturally.
- Camera is CLOSE — medium or close-up. No distant wide shots where the subject appears small.
- Lighting is real and available: window light, lamp glow, phone screen glow. Raw and authentic, never studio-lit.
- TikTok/Reels energy: slightly imperfect, intimate feel, like genuine human footage.

═══ ANTI-GLITCH RULES ═══
- ONE person only. Never "they", "friends", "people". Always "a [age] [gender]".
- Simple stable background per shot: "softly blurred bedroom", "out-of-focus kitchen counter", "plain dark wall". No busy environments.
- No rapid motion, no split-screen, no text on screen, no graphics, no captions.
- Smooth realistic human movement only — no action sequences, no dramatic gestures that are hard to render.

═══ NEGATIVE PROMPT HEADING ═══
After the script shots, add a "NEGATIVE PROMPT:" line. Always include: "multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, cleavage, tight outfit, wide shot with small subject" plus 2–3 scene-specific additions.

═══ FORMAT ═══
- Respond with valid JSON only. No markdown, no explanation outside JSON.
- script: plain text 600–900 characters for ${dur}s (longer duration = more content). Shot labels and "NEGATIVE PROMPT:" inside script value.
- suggestedDuration: integer. For a ${dur}s request, this MUST be ${dur}.

{"script": "...", "suggestedDuration": <integer>}`;

  const userPrompt = `MARKETING ANGLE: ${angle.name}
CORE MESSAGE: ${angle.core_message}
EMOTIONAL TERRITORY: ${emotionalTerritory}
ANGLE FORMAT: ${angle.format?.style || 'video'}, ${lengthSeconds}${creativePrinciple}

CREATIVE DIRECTIONS (choose the most cinematically powerful for this audience):
${creativeDirections}

MESSAGING EXAMPLES (tone reference only — do NOT copy verbatim, find your own words):
${messagingExamples}

TARGET AUDIENCE: ${audience || 'Not specified — invent a vivid, specific persona that fits this angle\'s emotional territory. Make it fresh and unexpected; avoid generic gamer stereotypes. The persona should feel like a real person with a real reason to connect with this game.'}
TARGET DURATION: ${dur}s (suggestedDuration must be 4–${dur})

GAME CONTEXT:
${gameContext}

STRONG EXAMPLE of a dense 3-shot ${dur <= 8 ? '8s' : '15s'} story script — match this level of detail, the visual hook/close structure, and include the negative prompt:
${dur <= 8
  ? '"SHOT 1 — VISUAL HOOK (wide, 3s): Dim bedroom, cold blue glow. A 27-year-old man in a plain t-shirt sits on the bed edge, shoulders drawn in, jaw tight — no dialogue, no phone movement. He\'s completely still, staring forward. The posture says everything: he just lost and he\'s not done. A single sharp exhale through the nose. The tension is visual — not a word spoken. SHOT 2 — SPEAK + CLOSE (medium, 5s): He lifts his chin and looks straight into the camera, voice quiet but iron-steady, like it\'s a promise: \\"Blitz of Battle — three minutes, full PvP. I lost that match. I\'m already in the next one.\\" Then: jaw unclenches. A slow breath out — the ghost of a smile arrives. Eyes say he already knows how it ends. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, defeat screen, surrender UI"'
  : '"SHOT 1 — VISUAL HOOK (wide, 5s): Softly blurred living room, late evening lamp light. A 30-year-old woman in a loose grey hoodie sits cross-legged on the couch, arms folded, phone face-down on the cushion beside her — untouched. Brow slightly furrowed, eyes fixed ahead. No dialogue. Her stillness tells the story: five losses, one after the other, and she\'s still here. The light is warm but her expression is iron. SHOT 2 — MAIN DIALOGUE (medium, 6s): She faces the camera directly — eyes serious, voice low and deliberate, like she\'s sharing something she had to earn: \\"I went on a five-loss streak in Blitz of Battle. Five. I didn\'t quit — I watched what I was doing wrong. Now I play every match like I already know I\'m going to win.\\" She leans slightly forward, jaw set, absolutely certain. SHOT 3 — VISUAL CLOSE (close-up, 4s): No words. A beat of stillness — then the corner of her mouth lifts. Just barely. Chin rises a fraction. She exhales through her nose. The kind of confidence that doesn\'t need to announce itself. Warm lamp light on her face, intimate and honest. The frame holds. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, tight outfit, cleavage, victory screen close-up"'
}

Write the Seedance prompt now. The viewer must FEEL the emotional territory, not just understand it. JSON only.`;
