...
export async function extractLastFrame(videoPath, outputJpg) {
  ...
  // fluent-ffmpeg on Windows can mangle paths that contain spaces.
  // -sseof -0.1 seeks 0.1s from the end of the file (output seek) — more reliable than
  // input-seek (-ss before -i) which can overshoot past the last keyframe and produce 0 frames.
  return new Promise((resolve, reject) => {
    const args = [
      '-y',                        // overwrite output without asking
      '-sseof', '-0.1',            // seek 0.1s from end (output-side, always lands on a real frame)
      '-i', videoPath,             // input file (passed as separate arg — no quoting issues)
      '-vframes', '1',             // grab exactly 1 frame
      '-q:v', '2',                 // JPEG quality
      outputJpg,                   // output file (separate arg)
    ];
