...
━━━ WHAT THIS IS ━━━
A MINI MOVIE. 3 scenes, 3 different locations, a complete story arc. NOT a talking-head. NOT a testimonial. Characters never address the camera. The story sells the game — not a monologue.

━━━ HOOK RULE — NON-NEGOTIABLE ━━━
SCENE 1 must open on a HIGH-EMOTION CLOSE-UP that makes a viewer STOP scrolling in the first second. Not passive. Not calm. The character is already mid-reaction — jaw dropped, eyes wide, fist clenched, shocked, furious, or laughing uncontrollably. The viewer must ask "what just happened?" before anything is explained.
BANNED for Scene 1: sitting calmly, walking, eating, looking around, staring blankly, "watching TV", slow reveals. Every Scene 1 must START already in a peak emotional moment.

━━━ STORY STRUCTURE (${dur}s total) ━━━
SCENE 1 — HOOK (${s1n}s): Extreme close-up. Character mid-peak-emotion. No context yet — pure visceral reaction. Different location from Scene 2. No dialogue. Snap zoom or tight static. Face fills frame.
SCENE 2 — STORY (${s2n}s): Context arrives — the game is playing. Same or new character. Blitz of Battle is mentioned casually by name — spoken to another person, or muttered to themselves (e.g. "Blitz of Battle's got me hooked", "I can't stop playing Blitz of Battle"). ONE short natural line only. Phone if shown: LANDSCAPE HORIZONTAL held sideways like a console controller, NOT portrait, screen facing viewer, soft glow on face. Energy rising.
SCENE 3 — PAYOFF (${s3n}s): The consequence lands. A rival's reaction. A trophy screen. A notification. A smirk. No dialogue. High energy close-up. Music peaks then cuts.

━━━ SCENE RULES ━━━
- EACH scene = a DIFFERENT physical location
- Dialogue: characters speak to each other or themselves only — never to camera
- Max 1 short spoken line per scene — natural, not scripted-sounding
- Emotion shown through FACE and BODY, not words
- Western characters, age 18–35, casual clothing

━━━ PHONE RULE — CRITICAL ━━━
${showMobileScreen
  ? 'PHONE MUST BE HELD LANDSCAPE (sideways, wide, like a Nintendo Switch). Write EXACTLY: "phone gripped two-handed in LANDSCAPE orientation — held sideways, wide-axis horizontal like a game controller, screen side facing camera, soft screen glow lighting face from below, face FULLY VISIBLE above the device." NEVER portrait. NEVER vertical phone. NEVER raised to eye level. NEVER blocking face. NO readable game UI on screen — soft glow only.'
  : 'No phone in frame. Story told through environment, faces, and body language only.'}

━━━ EACH SCENE MUST INCLUDE ALL 9 FIELDS ━━━
1. DURATION: "Xs"
2. SETTING: exact physical location
3. CAMERA: snap zoom / tight static / handheld close-up / over-shoulder — one specific choice
4. LIGHTING: light source + color temperature
5. CHARACTER: age, Western appearance, specific clothing
6. ACTION: body action — already in motion, concrete, one beat
7. DIALOGUE (if any): exact words + delivery + who spoken to
8. MOOD + COLOR GRADE: one phrase
9. MUSIC: tempo, genre feel, volume level

━━━ MUSIC ARC ━━━
Scene 1: near-silent tension or shock — sparse pulse, no melody
Scene 2: energy builds — beat drops as game moment hits, mid-high volume
Scene 3: peak hit then fade — punchy bass drop, short swell, silence

━━━ NEGATIVE PROMPT ━━━
End with: "NEGATIVE PROMPT: portrait phone, vertical phone, phone held vertically, character speaking to camera, direct-to-camera monologue, back of phone visible, phone rear camera, phone raised to eye level, phone blocking face, readable game UI on screen, gameplay map visible, talking head format, text overlays, blurry faces, distorted hands, slow motion, revealing clothing"

━━━ OUTPUT FORMAT ━━━
Valid JSON only. No markdown fences.
{"script": "SCENE 1: [all 9 fields]... SCENE 2: ... SCENE 3: ... NEGATIVE PROMPT: ...", "suggestedDuration": ${dur}}
script: 900–1300 characters. suggestedDuration: exactly ${dur}.`;

  // Strong cinematic example with correct phone orientation, hook, and game name
  const exampleScript = `{"script": "SCENE 1: 4s. Apartment bathroom, late night. Snap zoom into extreme tight close-up on face, locked off. Single bare bulb overhead, harsh white light. Western male, 24, stubble, grey t-shirt — jaw literally dropped, eyes wide in pure disbelief, hand over mouth. Frozen mid-reaction. Viewer has zero context — just that SOMETHING just happened. No dialogue. High contrast cold grade, face sharp everything else black. MUSIC: dead silence then a single low sub-bass pulse, near-inaudible. SCENE 2: 6s. Same guy, living room couch, warm lamp glow. Handheld close-up slight shake. He's hunched forward gripping phone in LANDSCAPE orientation — held sideways wide-axis like a game controller, screen side facing camera, soft blue glow lighting his face from below, face fully visible above the device. Friend off-screen: \\"What was THAT?\\" He exhales sharp, shaking his head: \\"Blitz of Battle just broke me. Three minutes and I'm done.\\" Disbelieving laugh, eyes still locked on screen. Golden warm grade, shallow bokeh. MUSIC: trap hi-hat and kick drop hard at start of line, driving rhythm, volume rises through the exchange. SCENE 3: 3s. Same couch, seconds later. Tight close-up. Blue screen glow. His thumb already moving — queuing the next match. The corner of his mouth pulls up. One slow blink. He's gone again. MUSIC: punchy bass hit on the smirk, single bright synth note, cuts to silence. NEGATIVE PROMPT: portrait phone, vertical phone, phone held vertically, character speaking to camera, direct-to-camera monologue, back of phone visible, phone rear camera, phone raised to eye level, phone blocking face, readable game UI on screen, gameplay map visible, talking head format, text overlays, blurry faces, distorted hands, slow motion, revealing clothing", "suggestedDuration": 13}`;

  const premiseLine = premise
    ? `STORY PREMISE: ${premise}\nBuild a 3-scene cinematic story around this. Scene 1 MUST open on an extreme emotional reaction — mid-peak, no build-up.`
    : `No premise given. Invent a story. Avoid: gamer in bedroom, person sitting calmly. Use real life — office worker, dad, athlete, tradesman, student. Scene 1 MUST open mid-peak-emotion — shock, disbelief, fury, hysterical laughter. No slow starts.`;

  const userPrompt = `${premiseLine}

${angleLine}

DURATION: ${dur}s total across 3 scenes.

GAME CONTEXT (embed naturally in story — the name "Blitz of Battle" must appear casually in Scene 2 dialogue):
${gameContext}

EXAMPLE — match this exact structure. Note: Scene 1 opens on pure emotion with zero context, Scene 2 has the game name spoken naturally, phone is LANDSCAPE not portrait:
${exampleScript}

Write the Seedance prompt now. JSON only. Use SCENE labels.`;
