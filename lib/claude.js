...
export async function generateQuickVideoScript({ audience, productPoint, duration, gameContext, showMobileScreen = true }) {
  ...
  SHOT 1 (~${Math.round(dur * 0.55)}s): Opening — character mid-activity, emotional state visible, building energy.
  SHOT 2 (~${Math.round(dur * 0.45)}s): Close-up — character speaks directly to camera, full dialogue delivered, reaction beat closes.`
    : dur <= 10
    ? `SHOT STRUCTURE (3 shots, total = exactly ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.35)}s): Wide/medium — character in environment, actively doing something, emotional state fully visible.
  SHOT 2 (~${Math.round(dur * 0.4)}s): Medium — character turns to camera, delivers most of the dialogue, emotion peaks.
  SHOT 3 (~${Math.round(dur * 0.25)}s): Close-up face — finishes dialogue, strong reaction beat, expression lingers.`
    : `SHOT STRUCTURE (3 shots, total = exactly ${dur}s — FILL EVERY SECOND):
  SHOT 1 (~${Math.round(dur * 0.33)}s): Establishing — character clearly mid-activity in environment, emotional setup rich with detail, audience feels the world.
  SHOT 2 (~${Math.round(dur * 0.4)}s): Medium — character faces camera fully, delivers ALL dialogue lines (at least 2 sentences), emotion is full and sustained, never trails off early.
  SHOT 3 (~${Math.round(dur * 0.27)}s): Close-up — reaction beat with visible emotion, a second expression shift, scene breathes and closes — DO NOT end abruptly.`;

  const systemPrompt = `You are a world-class UA video scriptwriter for Seedance 2.0 (portrait 9:16, native audio, ${dur}s target).
Your script is the EXACT generation prompt fed to the AI model. Seedance natively supports multi-shot video — describe ${shotCount} distinct shots that cut together like a real ad.
...
- Each shot has its own: location/framing, character action, emotional state, and tone.
- Each shot must be DENSE — 2–3 things happening per shot (action + emotion + vocal delivery). No shot is ever just "character looks at camera."
...
INSTEAD: every moment is one of these — speaking, reacting, laughing, exhaling with satisfaction, leaning in, eyes lighting up. Nothing else.
...
- Simple stable background per shot: "softly blurred gym wall", "out-of-focus kitchen", "plain dark background".
...
After the script shots, add a "NEGATIVE PROMPT:" line. Always include: "multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, cleavage, tight outfit" plus 2–3 scene-specific additions.
...
{"script": "...", "suggestedDuration": <integer>}`;
  ...
  const userPrompt = `TARGET AUDIENCE: ${audience}
...
${dur <= 8
  ? `"SHOT 1 (medium, 4s): Softly blurred kitchen, warm morning light. A 34-year-old mom in a hoodie leans against the counter, coffee mug in both hands, eyes already bright — she's been looking forward to this moment. She lets out a quiet satisfied exhale. SHOT 2 (medium, 3s): She straightens up, looks straight into the camera, voice hushed and fast like she's sharing a secret: \\"I get a full PvP match in Blitz of Battle while my kid's eating breakfast — three minutes, in and out, I win more than I should.\\" SHOT 3 (close-up, 1s): She bites her lip to hide her grin, eyes dancing. Warm candid light throughout. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, messy kitchen, children in frame"`
  : `"SHOT 1 (wide, 5s): Softly blurred living room, late evening. A 32-year-old dad in a grey hoodie sits on the couch, leaning forward with elbows on knees — eyes locked forward, clearly mid-thought, barely moving. There's a stillness to him like he's been waiting all day for this. He exhales slowly through his nose. SHOT 2 (medium, 6s): He looks directly into the camera, voice calm and deliberate, like he's explaining something obvious to someone who doesn't get it yet: \\"I played MOBAs for years. Forty-minute matches, surrender votes, teammates raging. Then I found Blitz of Battle — three minutes, full PvP, all the skill, none of the garbage.\\" He leans back slightly, satisfied. Warm low light catches his face. SHOT 3 (close-up, 4s): His expression settles into a quiet, earned smile. He glances down once — a small nod — then back up. Eyes say he's already thinking about the next match. Warm intimate lighting. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, busy background, children in frame"`
}`
...
export async function generateQuickVideoStoryScript({ angle, audience, duration, gameContext, showMobileScreen = true }) {
  ...
  const shotGuidance = dur <= 6
    ? `SHOT STRUCTURE (2 shots, total = exactly ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.55)}s): Opening — character mid-activity, angle's tension established immediately, emotion visible.
  SHOT 2 (~${Math.round(dur * 0.45)}s): Close-up — character speaks directly to camera, emotional payoff delivered, reaction beat closes.`
    : dur <= 10
    ? `SHOT STRUCTURE (3 shots, total = exactly ${dur}s):
  SHOT 1 (~${Math.round(dur * 0.35)}s): Wide/medium — character in their world, angle's emotional tension introduced, situation feels real.
  SHOT 2 (~${Math.round(dur * 0.4)}s): Medium — character turns to camera, story arc develops, main dialogue delivered with full emotion.
  SHOT 3 (~${Math.round(dur * 0.25)}s): Close-up — emotional payoff, reaction beat, tension resolved through expression.`
    : `SHOT STRUCTURE (3 shots, total = exactly ${dur}s — FILL EVERY SECOND):
  SHOT 1 (~${Math.round(dur * 0.33)}s): Establishing — character mid-activity in environment, angle's tension introduced richly with emotional texture and physical detail.
  SHOT 2 (~${Math.round(dur * 0.4)}s): Medium — character faces camera, delivers ALL dialogue (minimum 2 sentences), emotion sustained through the full shot, never trails off early.
  SHOT 3 (~${Math.round(dur * 0.27)}s): Close-up — reaction beat, a second visible emotion shift, scene breathes fully before closing — DO NOT end abruptly.`;
...
Your script is the EXACT generation prompt fed to the AI model. Seedance natively supports multi-shot video — describe ${shotCount} distinct shots that tell a story with real emotional arc.
...
${shotGuidance}
- Label each shot clearly: "SHOT 1:", "SHOT 2:", "SHOT 3:" — Seedance reads these as cut points.
- Each shot has its own: location/framing, character action, emotional state, and tone description.
- Each shot must be DENSE — 2–3 things happening (action + emotion + vocal delivery). No shot is just "character looks at camera."
- The narrative arc must build across shots — each one advances the story, not repeating the same moment.

═══ VOICE & EMOTION DIRECTION ═══
Seedance generates audio from your prompt. Emotionally flat prompts produce flat, robotic voices:
- Describe the character's inner emotional state ENTERING each shot: "barely containing excitement", "conflicted but lit up", "pretending to be casual but clearly thrilled"
...
INSTEAD: every moment is one of — speaking, reacting, laughing, exhaling with satisfaction, leaning in with intensity, eyes lighting up, jaw tightening with tension. Nothing else.
...
STRONG EXAMPLE of a dense 3-shot ${dur <= 8 ? '8s' : '15s'} story script — match this level of detail and include the negative prompt:
${dur <= 8
  ? `"SHOT 1 (wide, 3s): Dim bedroom, blue light glow. A 27-year-old man in a plain t-shirt sits on the bed edge, jaw tight, shoulders drawn in — he just lost and he knows it. A sharp exhale through the nose. He doesn't look away from where the screen would be. SHOT 2 (medium, 3s): He lifts his chin and looks straight into the camera, voice quiet but iron-steady: \\"Blitz of Battle — three minutes, full PvP. I lost that match. I'm already in the next one.\\" SHOT 3 (close-up, 2s): Jaw unclenches. A slow breath out — the ghost of a smile. Eyes say he already knows how this ends. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, defeat screen, surrender UI"`
  : `"SHOT 1 (wide, 5s): Softly blurred living room, late evening lamp light. A 30-year-old woman in a loose grey hoodie sits cross-legged on the couch — phone resting on her knee, arms folded, brow furrowed. She's been losing and she's not okay with that. She exhales sharply through her nose, sits up straighter, shakes her head once like she's resetting. The energy in the room shifts. SHOT 2 (medium, 6s): She looks directly into the camera — eyes serious, voice low and deliberate, like she's telling you something she figured out the hard way: \\"I went on a five-loss streak in Blitz of Battle. Five. I didn't quit — I watched what I was doing wrong. Now I play every match like I already know I'm going to win.\\" She leans forward slightly, jaw set. SHOT 3 (close-up, 4s): A beat of stillness — then the corner of her mouth lifts. Just barely. She exhales through her nose, chin lifting. The kind of confidence that doesn't need to announce itself. Warm lamp light on her face, intimate and honest. NEGATIVE PROMPT: multiple people, text overlays, split screen, rapid camera movement, blurry faces, distorted hands, phone in portrait mode, revealing clothing, tight outfit, cleavage, victory screen close-up"`
}`
