 ...
 */
export async function planScenes(storyText, durationRange, gameContext, voiceDesc, heroDesc, images = {}, allowedDurations = [], pipelineMode = 'standard', heroesData = null, namedHeroes = []) {
...
SPECIFICALLY SELECTED HEROES FOR THIS VIDEO (${namedHeroes.length} hero${namedHeroes.length > 1 ? 's' : ''} chosen by the user — MUST appear in the video):
${details}

CRITICAL: These heroes were explicitly chosen. Their visual_description above is the AUTHORITATIVE source for how they look. Write every image prompt using their visual_description as precisely and faithfully as if a reference image were attached — same outfit, same colors, same weapon, same proportions. Name the hero explicitly in every image prompt (e.g. "Halyx, the Master Assassin, bare-chested with gray arm guards and katana...").`;
  }
  ...
  if (namedHeroes?.length > 0 && hasHeroImage) {
    namedHeroesSection = namedHeroesSection.replace(
      'Their visual_description above is the AUTHORITATIVE source',
      'Reference images for these heroes are attached — use them alongside the visual descriptions. The visual_description above is the AUTHORITATIVE source'
    );
  }
...
The voice must sound like it belongs in a AAA game trailer — confident, commanding, exciting, selling the game.`;
  ...
  const heroRefText = hasMultipleHeroes
    ? `- ${heroCount} HERO reference images have been provided (see attached). Each image represents a DISTINCT character. Study every hero carefully — note each character's exact appearance, outfit, colors, proportions, art style, and any weapons/items they carry. The story features MULTIPLE characters and scenes may include any combination of them.`
    : (hasHeroImage
      ? '- A single HERO reference image has been provided (see attached). Study it meticulously — memorize the character\'s exact face, body proportions, hair, outfit/clothing (every detail — colors, patterns, accessories), and any weapon or item they carry. The ENTIRE story revolves around this ONE hero. CRITICAL CHARACTER LOCK: the hero\'s physical appearance, clothing, and weapon MUST remain 100% identical across ALL scenes — same outfit, same colors, same weapon design, zero alterations. CRITICAL IMAGE RULE: image prompts must contain ONLY this hero character and NOTHING ELSE. Absolutely NO teammates, partners, allies, silhouettes, shadowed figures, or background characters of any kind — even if the story mentions a teammate. A teammate\'s presence may only be implied through environmental cues (dust trails, energy residue, off-screen sound cues in the video prompt), but ZERO additional human/character figures may appear in any image frame.'
      : '');
...
IMPORTANT: EVERY scene imagePrompt MUST start with a direction label in brackets — [FRONT], [BACK], [LEFT-SIDE], [RIGHT-SIDE], [LOW-FRONT], [HIGH-ABOVE], [THREE-QUARTER-FRONT], [THREE-QUARTER-BACK], or [DUTCH-ANGLE]. No two scenes may share the same label.`
    : `{
  "voiceOverCharacteristics": "He says in the voice of a mature man, deep commanding voice, intense and electrifying tone, faster pacing",
  "scenes": [
    {
...
IMPORTANT: EVERY imagePrompt and imageBPrompt MUST start with a direction label in brackets — [FRONT], [BACK], [LEFT-SIDE], [RIGHT-SIDE], [LOW-FRONT], [HIGH-ABOVE], [THREE-QUARTER-FRONT], [THREE-QUARTER-BACK], or [DUTCH-ANGLE]. No two prompts may share the same label — every shot must be from a genuinely different physical direction.`;
...
${hasMultipleHeroes
  ? '- When useHeroRef is true, describe the hero(es) in the scene matching their respective reference images exactly. EXPLICITLY include in every prompt: "following the provided character reference image exactly — preserve the exact face, body proportions, outfit, clothing details, colors, hairstyle, and weapon — do NOT alter any aspect of the character\'s appearance or equipment"'
  : (hasHeroImage ? '- When useHeroRef is true, describe the hero matching the reference image exactly. EXPLICITLY include in every imagePrompt: "following the provided hero reference image exactly — preserve the character\'s exact face, body proportions, outfit, clothing colors and details, hairstyle, and weapon — do NOT change, add, or remove any element of the character\'s appearance or equipment"' : '- Include the hero character description in the image prompt')}
...
- CHARACTER APPEARANCE LOCK: The character's physical appearance is FROZEN and must be identical across every scene. Same face, same hair, same outfit and clothing details, same weapon design and colors. Do NOT add any armor, accessories, capes, gear, or items not present in the reference image. Do NOT remove or modify any existing element. Only the camera angle and background change between scenes.
- WEAPON LOCK: If the character holds a weapon in the reference image, that EXACT weapon must appear in every scene where the character is shown — same weapon design, same colors, same style. Never swap, upgrade, remove, or alter the weapon in any way.
- CRITICAL VARIETY — GENUINELY DIFFERENT DIRECTION EVERY SCENE: Each image MUST show the character from a completely different physical direction — not just a different zoom or composition from the same side. Rotate through these labeled directions, using each only once per video:
  ...
  START every imagePrompt with the direction label in brackets, e.g. "[LEFT-SIDE] Following the provided hero reference image exactly — ..."
...
IMPORTANT: You MUST create at least 2 scenes. The total of all durations MUST be between ${durMin} and ${durMax} seconds.`;
...
- MUST INCLUDE: Leaning slightly forward toward a large professional black studio condenser microphone on a boom arm, mid-sentence with mouth slightly open, EYES LOOKING DIRECTLY INTO THE CAMERA (direct eye contact with the viewer/audience), one hand raised gesturing expressively
...
- BACKGROUND (CRITICAL — NOT white): Detailed gaming/streaming room setup with:
  * Dual monitors behind showing colorful game footage or stream overlays
  * RGB LED strips casting colored ambient light (purple, blue, cyan — vary the palette)
  * Acoustic foam panels or soundproofing on walls
  * Pop filter near the microphone
  * Visible streaming equipment (camera, ring light, etc.)
  * Gaming posters, figurines, or collectibles on shelves
  * Desk with keyboard, mouse, energy drink
- Faint RGB glow reflecting off the left side of face and clothing from monitors
...
- Keep voice descriptors SHORT and SIMPLE — 2-4 words each, no compound adjectives:
  ...
  * TONE: pick ONE e.g. "confident tone" / "excited tone" / "relaxed tone" / "intense tone"
  * PACING: pick ONE e.g. "faster pacing" / "moderate pacing" / "energetic pacing" / "steady pacing"
...
- GOOD example: "He says in the voice of a young man, sharp clear voice, confident tone, faster pacing: \\"dialogue\\""
...
- After the voice line, describe: subtle camera push-in toward the podcaster, natural hand gestures, direct eye contact maintained with the camera throughout, engaged facial expressions, slight body lean changes
- The podcaster is SEATED and STATIONARY — no dramatic camera movements. Keep it natural: gentle push-in, subtle rack focus shifts, natural conversational gestures
...
- Allowed durations: ${allowedDurations.join(', ')} seconds. Maximum is ${allowedDurations[allowedDurations.length - 1]}s.
- Speaking rate: ~2.5 words per second${preferredDuration ? `
- USER HAS REQUESTED A SPECIFIC DURATION: ${preferredDuration}s. You MUST set suggestedDuration to ${preferredDuration}. Write the script to fit exactly ${preferredDuration}s (~${Math.floor(preferredDuration * 2.5)} words).` : `
- Maximum script length: ${allowedDurations[allowedDurations.length - 1]} × 2.5 = ${allowedDurations[allowedDurations.length - 1] * 2.5} words MAX
- If writing a script: keep it under ${Math.floor(allowedDurations[allowedDurations.length - 1] * 2.5)} words so it fits in the video
- If the user provided a script that exceeds ${Math.floor(allowedDurations[allowedDurations.length - 1] * 2.5)} words: TRIM it to fit within the max duration — keep the most impactful lines, end with a strong CTA`}
- suggestedDuration MUST be one of: ${allowedDurations.join(', ')} — ${preferredDuration ? `use ${preferredDuration}` : `pick the one that fits the (trimmed) script word count`}
