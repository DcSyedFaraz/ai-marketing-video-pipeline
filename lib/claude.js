...
export async function generateQuickVideoScript({ audience, productPoint, duration, gameContext, showMobileScreen = true }) {
  ...
  const phoneRule = showMobileScreen
    ? 'Phone grip: two-handed horizontal landscape, screen facing camera, Blitz of Battle gameplay visible. Character glances at screen once (under 0.5s) then eyes snap back to camera. Never describe finger movements or UI elements.'
...
━━━ EACH SHOT MUST SPECIFY ALL 8 FIELDS ━━━
...
━━━ NEGATIVE PROMPT ━━━
End every script with: "NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing"

━━━ OUTPUT FORMAT ━━━
Valid JSON only. No markdown fences.
{"script": "SHOT 1: [all 8 fields]... SHOT 2: ... NEGATIVE PROMPT: ...", "suggestedDuration": ${dur}}
script must be 700–1100 characters. suggestedDuration must be ${dur}.`;

  // Strong example scaled to duration
  const uaExample = dur <= 8
    ? `{"script": "SHOT 1: 9:16 vertical, 3s. Static close-up, handheld micro-shake. Harsh overhead light, cold blue. Western male, 24, dark stubble, grey crewneck, jaw tight, eyes locked forward — already mid-reaction, not moving into it. No dialogue. Expression: caught between rage and focus. High contrast, cold grade. Subject sharp, background wall blurred. SHOT 2: 9:16 vertical, 5s. Slight push-in to close-up. Warm lamp glow from left, soft bokeh background. Same guy, leans into frame, fires off rapid: \\"I was losing every match for a week. Switched to Blitz of Battle — three-minute rounds, pure skill, no filler. Now I don't put it down.\\" Grins at end, eyes bright. Warm golden grade. Shallow depth of field. NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing", "suggestedDuration": 8}`
    : `{"script": "SHOT 1: 9:16 vertical, 4s. Locked-off medium, slight handheld shake. Cold blue phone-screen glow, dim room. Western male, 27, light beard, black hoodie, sitting upright — elbows on knees, phone held two-handed horizontal landscape, Blitz of Battle gameplay facing camera. Eyes flick from screen to lens — jaw set, competitive. No dialogue. High contrast, cool grade, shallow bokeh background. SHOT 2: 9:16 vertical, 7s. Static close-up, face filling vertical frame. Warm lamp glow, subject sharp, background blurred. Same guy, stares direct to camera, rapid-fire delivery, barely containing it: \\"Console games waste four hours of your life. Blitz of Battle — two versus two, three minutes, you either outthink them or you lose. That's it. I'm addicted.\\" Quick sharp grin after the last word. Warm golden grade. SHOT 3: 9:16 vertical, 4s. Snap zoom out to medium. Same cold blue glow returns. He drops his chin once — a single decisive nod — then looks back at the screen. No words. Done. High contrast, desaturated edges. NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing", "suggestedDuration": 15}`;
...
export async function generateQuickVideoNarrativeScript({ premise, angle, duration, gameContext, showMobileScreen = true }) {
  ...
  const phoneRule = showMobileScreen
    ? 'Phone grip: two-handed horizontal landscape, screen facing camera, Blitz of Battle gameplay visible. One quick glance (under 0.5s) then eyes back to camera. Never describe finger movements or on-screen UI.'
    : 'No phone in frame. Story told through character actions and dialogue only.';

  const angleLine = angle
    ? `MARKETING ANGLE: "${angle.name}" — ${angle.core_message}. The story must make the viewer FEEL this through action and dialogue — not state it like an ad.`
    : 'No specific angle. Game name "Blitz of Battle" must be spoken in Shot 2 or Shot 3.';

  const s1n = Math.round(dur * 0.27);
  const s3n = Math.round(dur * 0.27);
  const s2n = dur - s1n - s3n;

  const systemPrompt = `You write Seedance 2.0 generation prompts for narrative UGC-style mobile game ads. Western male audience 18–35. TikTok/META vertical feed. 9:16 portrait 720×1280.

WHAT YOU ARE WRITING: Not a screenplay. A GENERATION PROMPT — the exact text fed to an AI video model. Every word must describe something visually generatable and concretely renderable.

━━━ EACH SHOT MUST SPECIFY ALL 8 FIELDS ━━━
1. ASPECT RATIO + DURATION: "9:16 vertical, Xs"
2. CAMERA: handheld, snap zoom, static close-up, locked-off medium — specific, one choice
3. LIGHTING: cold blue glow, warm lamp, harsh overhead, window side-light — concrete, not poetic
4. CHARACTER: age (18–35), Western male, specific clothing — one person per shot max
5. ACTION: what the body is doing — already in position, simple, stable, one motion
6. DIALOGUE (if any): exact words in quotes + HOW delivered: rapid-fire, deadpan, smirking, breathless
7. MOOD + COLOR GRADE: warm golden, cold desaturated, high contrast — one phrase
8. DEPTH OF FIELD: shallow bokeh / subject sharp background blurred / deep focus

━━━ NARRATIVE STRUCTURE (${dur}s total) ━━━
SHOT 1 (${s1n}s) — SETUP: Show the PROBLEM or TENSION without dialogue. Character already mid-reaction. Hook lands in 2 seconds — strong emotion on face, body language telling the story. Not a calm establishing shot.
SHOT 2 (${s2n}s) — TURN: Game enters. "Blitz of Battle" spoken here OR in Shot 3. Phone (if shown) two-handed landscape. Dialogue short and punchy — rapid-fire delivery.
SHOT 3 (${s3n}s) — PAYOFF: Resolution. If game name not in Shot 2, it MUST appear here. Fast emotional close — a grin, a sharp exhale, one decisive beat. No new info.

━━━ IRON-CLAD RULE ━━━
"Blitz of Battle" MUST be spoken out loud in Shot 2 or Shot 3. Not implied. Not on screen. SAID.

━━━ PACING — NON-NEGOTIABLE ━━━
Fast cuts. High energy. Every shot is alive.
BANNED: "slowly", "pauses", "gazes", "lingers", "hesitates", "sits quietly", "takes a moment"
Dialogue delivery: rapid-fire / grinning / deadpan / breathless / fired up — never "says calmly"
Actions: simple, contained, already in motion — no "walks over", "stands up", "reaches for"

━━━ WESTERN CHARACTERS ONLY ━━━
Western appearance only. Casual clothing. Women: modest — hoodie, jeans, t-shirt. Men: t-shirt, hoodie, casual shirt.

━━━ PHONE RULE ━━━
${phoneRule}

━━━ NEGATIVE PROMPT ━━━
End with: "NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing"

━━━ OUTPUT FORMAT ━━━
Valid JSON only. No markdown fences.
{"script": "SHOT 1: [all 8 fields]... SHOT 2: ... SHOT 3: ... NEGATIVE PROMPT: ...", "suggestedDuration": ${dur}}
script: 750–1100 characters. suggestedDuration: exactly ${dur}.`;

  const exampleScript = `{"script": "SHOT 1: 9:16 vertical, 4s. Locked-off medium, handheld micro-shake. Cold blue TV glow, dim room. Western male, 26, short hair, white t-shirt, sits on couch — controller in both hands, shoulders forward, jaw clenched. Just lost. No dialogue. Face says everything: frustrated, not done. High contrast cold grade, subject sharp background blurred. SHOT 2: 9:16 vertical, 7s. Static close-up, slight push-in. Warm lamp light from left, shallow bokeh. Same guy, drops controller beside him, picks up phone two-handed horizontal landscape. Screen faces camera — Blitz of Battle visible. Snaps look to camera, rapid-fire: \\"Console match takes an hour. Blitz of Battle — two vs two, three minutes, I'm already in the next one.\\" Sharp grin. Golden warm grade. SHOT 3: 9:16 vertical, 4s. Snap zoom to tight close-up. Cold blue returns. Eyes locked forward, corners of mouth pulling up — one decisive nod. No words. Done. High contrast, desaturated. NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing", "suggestedDuration": 15}`;

  const premiseLine = premise
    ? `STORY PREMISE: ${premise}`
    : `AUDIENCE + PREMISE: Western male 18–35. Invent a story — unexpected scenario, avoid "gamer in bedroom". Something real: office worker, athlete, dad, tradesman. The game solves a real frustration.`;
...
export async function generateQuickVideoStoryScript({ angle, audience, duration, gameContext, showMobileScreen = true }) {
  ...
  const phoneRule = showMobileScreen
    ? 'Phone grip: two-handed horizontal landscape, screen facing camera, Blitz of Battle gameplay visible. One quick glance (under 0.5s) then eyes back to camera. Never describe finger movements or UI.'
...
script: 700–1100 characters. suggestedDuration: exactly ${dur}.`;
  ...
  const storyExample = dur <= 8
    ? `{"script": "SHOT 1: 9:16 vertical, 3s. Locked-off close-up, handheld micro-shake. Cold blue phone-screen glow. Western male, 24, black stubble, grey crewneck — elbows on knees, jaw set, eyes laser-focused forward. No dialogue. Already mid-tension, not settling into it. High contrast cold grade, subject sharp, wall blurred. SHOT 2: 9:16 vertical, 5s. Static medium, slight push-in. Warm lamp from left, shallow bokeh. Same guy, snaps eyes to camera, rapid-fire: \\"I was throwing matches for days. Then Blitz of Battle — two vs two, three minutes, pure reads. Now I see the mistakes before they happen.\\" Sharp grin after. Golden grade. NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing", "suggestedDuration": 8}`
    : `{"script": "SHOT 1: 9:16 vertical, 4s. Locked-off medium, handheld micro-shake. Cold blue phone glow, dim room. Western male, 27, stubble, black hoodie — upright on couch, phone two-handed horizontal landscape, Blitz of Battle facing camera. Eyes flick to lens — jaw set, completely in it. No dialogue. High contrast, cold grade, shallow bokeh. SHOT 2: 9:16 vertical, 7s. Static close-up, push-in. Warm lamp glow, subject sharp, background blurred. Same guy, locks eyes on camera, rapid-fire: \\"Other MOBAs — four hours, one match, your team feeds and you lose ranked. Blitz of Battle: two vs two, three minutes, you outthink them or you don't. That's the whole game. I've played thirty matches today.\\" Grins at the end. Warm golden grade. SHOT 3: 9:16 vertical, 4s. Snap zoom to tight close-up. Cold blue returns. One hard nod — mouth shut, eyes alive. No words. High contrast, desaturated. NEGATIVE PROMPT: portrait mode phone, text overlays, split screen, blurry faces, distorted hands, screen UI readable, multiple phones, finger swiping visible, multiple people, slow motion, wide shot small subject, revealing clothing", "suggestedDuration": 15}`;
