...
/**
 * Generate a street-interview style Seedance 2.0 video prompt.
 * Strict guidelines: handheld spontaneous feel, short punchy dialogue,
 * "three minutes" punchline, lo-fi music structure, landscape phone as proof.
 */
export async function generateQuickVideoStreetInterviewScript({ hook, location, duration, gameContext }) {
  if (!ANTHROPIC_KEY) throw new Error('ANTHROPIC_API_KEY not set in .env');
  const client = new Anthropic({ apiKey: ANTHROPIC_KEY });
  const dur = Math.max(10, Math.min(15, parseInt(duration) || 12));

  const sceneCount = dur <= 10 ? 3 : 4;
  const s1 = Math.round(dur * 0.2);
  const sLast = Math.round(dur * 0.2);
  const sMid = dur - s1 - sLast;
  const s2 = sceneCount === 4 ? Math.round(sMid * 0.5) : sMid;
  const s3 = sceneCount === 4 ? sMid - s2 : 0;

  const timingLine = sceneCount === 4
    ? `SCENE 1 — HOOK (${s1}s) | SCENE 2 — DISCOVERY (${s2}s) | SCENE 3 — PROOF (${s3}s) | SCENE 4 — PUNCHLINE (${sLast}s)`
    : `SCENE 1 — HOOK (${s1}s) | SCENE 2 — DISCOVERY+PROOF (${sMid}s) | SCENE 3 — PUNCHLINE (${sLast}s)`;

  const locationLine = location
    ? `SETTING: ${location}.`
    : 'SETTING: Choose a specific outdoor urban location — sidewalk near food truck, bus stop bench, park path, coffee shop patio. The guest must have a believable reason to be standing still or idle.';

  const hookLine = hook
    ? `OPENING APPROACH: ${hook}`
    : 'OPENING APPROACH: Interviewer runs up to someone mid-activity — spontaneous energy, catching up, not composed.';

  const systemPrompt = `You write Seedance 2.0 generation prompts for STREET INTERVIEW mobile game ads. 9:16 portrait 720x1280. TikTok/META vertical feed.

━━━ FORMAT ━━━
A fake-spontaneous street interview. Interviewer approaches a stranger, discovers they play Blitz of Battle, the "three minutes" payoff lands naturally. Feels REAL — never scripted.

━━━ OPENING HOOK — SCENE 1 RULES (NON-NEGOTIABLE) ━━━
- Interviewer always MID-ACTION on frame one — running, jogging up, already approaching. NEVER standing still waiting.
- Running or approaching energy creates immediate spontaneous feel.
- Camera MOVES WITH interviewer — shaky, catching up, never composed or locked off.
- NO music in Scene 1 — street noise only until first genuine reaction lands.
- Interviewer: casual outfit — never formal, never branded.
- Guest caught MID-ACTIVITY — walking, waiting, scrolling phone, on lunch break.
- Guest pulls earbud out when stopped — makes the stop feel real.

━━━ DIALOGUE RULES (STRICT) ━━━
- ONE sentence per person per scene MAXIMUM.
- Every line UNDER 10 words.
- No clean turns — interviewer reacts BEFORE guest finishes, guest answers immediately without pause.
- No artificial beats or written pauses between lines.
- Dialogue flows continuously — feels like it was never scripted.
- SLOW and CLEAR delivery specified on EVERY spoken line.
- NEVER more than two lines of dialogue per scene.

━━━ CHARACTER RULES ━━━
- Interviewer: casual clothes, no branding, no microphone (NO foam/branded mic ever).
- Guest: caught mid-activity, slightly moving, never perfectly still.
- Both characters always slightly in motion — nobody stands perfectly still.
- Guest NEVER looks at camera — only at interviewer or phone.

━━━ PHONE HANDLING (CRITICAL) ━━━
- ALWAYS landscape orientation, two-handed grip.
- Screen ALWAYS facing outward toward interviewer and camera.
- Guest pulls phone NATURALLY mid-conversation — never on cue.
- Phone comes out as PROOF, not as a demo.
- Phone at lower chest / waist level — NOT raised to eye level.

━━━ ARENA DESCRIPTION (when phone screen shown) ━━━
Always describe the game screen as: "Isometric top-down mobile MOBA view — circular arena with low curved concrete barrier walls, dark teal-green grass surface, two concentric glowing ground rings one cyan one red, yellow diagonal glowing line across arena floor, small 3D heroes clashing center, floating damage numbers, purple circular ability buttons bottom right, health percentage HUD top corners, circular timer top center."

━━━ THE PUNCHLINE STRUCTURE (FINAL SCENE — NON-NEGOTIABLE) ━━━
- Always build to "three minutes" as the payoff.
- Interviewer repeats it back as a question — "three minutes?"
- Guest confirms with ZERO extra words — just "three minutes."
- Camera pulls back on this exchange to widen the emotional moment.
- Final beat: silent visual moment after the last line — no dialogue needed to close.

━━━ CAMERA BEHAVIOUR ━━━
- Handheld throughout — never fully stable.
- Loose framing — not perfectly composed or centered.
- Pull-back from phone screen to two-shot used as emotional beat.
- Camera never cuts cleanly — always feels continuous.
- NO slow motion anywhere.

━━━ MUSIC STRUCTURE ━━━
- Scene 1: NO music. Street noise only.
- Music enters on first genuine emotional reaction — a laugh, a surprise, a casual answer.
- Lo-fi soft beat throughout — never competing with dialogue.
- Beat softens or drops before the punchline lands.
- Single soft chord resolves on the final beat.
- Music NEVER louder than dialogue at any point.

━━━ SETTING RULES ━━━
- Location always has a reason for the person to be standing still or idle.
- Background always has natural movement — crowd, traffic, bus.
- Lighting always natural — warm afternoon or overcast daylight.
- No studio lighting, no controlled environment.

━━━ STORY ARC (${dur}s total) ━━━
${timingLine}

SCENE 1 — HOOK: Interviewer mid-run approaching guest. Guest caught mid-activity. Shaky handheld camera following. Street noise. No music. Guest pulls earbud out. Interviewer asks a quick question (under 10 words).
${sceneCount === 4
  ? `SCENE 2 — DISCOVERY: Guest casually mentions Blitz of Battle. Short rapid exchange. Music enters on the reaction. Handheld two-shot.
SCENE 3 — PROOF: Guest pulls phone landscape, shows the game as proof. Camera catches arena screen. Quick impressed reaction from interviewer.`
  : `SCENE 2 — DISCOVERY+PROOF: Guest mentions Blitz of Battle, pulls phone landscape as proof. Music enters. Camera catches arena on screen. Quick exchange.`}
SCENE ${sceneCount} — PUNCHLINE: "Three minutes?" / "Three minutes." Camera pulls back. Silent beat. Soft chord resolves. End.

━━━ EACH SCENE MUST INCLUDE ━━━
1. DURATION: "Xs"
2. SETTING: specific location detail + background movement
3. CAMERA: handheld behavior + framing
4. LIGHTING: natural source + warmth
5. CHARACTER: age, Western appearance, casual clothing, current action
6. ACTION: specific physical movement already happening
7. DIALOGUE (if any): exact words + "slow clear delivery" + who spoken to
8. MOOD: one phrase
9. MUSIC: what's playing (or silence/street noise)

━━━ NEGATIVE PROMPT (ALWAYS INCLUDE AT END) ━━━
NEGATIVE PROMPT: news microphone, foam microphone, branded microphone, portrait phone, vertical phone, back of phone visible, phone screen blank, colosseum, Roman arena, tall arena walls, giant heroes, side-view arena, theatrical reactions, robotic delivery, studio lighting, text overlays, finger gestures, pointing, slow motion, blurry faces, distorted hands, crowd reacting to interview, music before Scene 2, artificial pauses, staged positioning, phone raised to eye level, revealing clothing

━━━ OUTPUT FORMAT ━━━
Valid JSON only. No markdown.
{"script": "SCENE 1: ...SCENE 2: ...NEGATIVE PROMPT: ...", "suggestedDuration": ${dur}, "refHint": "Attach a landscape gameplay screenshot in Extra Refs — Seedance uses it for the phone screen in the proof scene."}
script: 900–1400 characters. suggestedDuration: exactly ${dur}.`;

  const exampleScript = sceneCount === 4
    ? `{"script": "SCENE 1 — 3s. Busy sidewalk near taco truck, warm afternoon sun. Handheld camera bouncing, catching up — interviewer jogging toward a guy leaning against a lamppost scrolling his phone. Natural street noise, traffic passing, people walking behind. Western male interviewer, 24, oversized grey hoodie, messy hair, no mic. Approaches at a jog. Western male guest, 27, black cap backwards, earbuds in, olive jacket, one foot on the wall — pulls right earbud out mid-approach. Interviewer, slightly out of breath, slow clear delivery: \\"What do you do when you got five minutes?\\" Guest glances up, half-smile. No music — street ambience only. Warm golden afternoon grade. SCENE 2 — 3s. Same spot, camera settles into loose handheld two-shot. Guest shrugs, casual, slow clear delivery: \\"I play Blitz of Battle.\\" Interviewer eyebrows up immediately — doesn't wait for the sentence to finish. Interviewer, overlapping, slow clear delivery: \\"Wait on your phone?\\" Lo-fi beat fades in soft under the reaction. Warm tones, shallow depth. SCENE 3 — 3s. Guest shifts weight, pulls phone from jacket pocket — landscape, two-handed grip, screen facing outward at lower chest. Isometric top-down mobile MOBA view — circular arena with low curved concrete barrier walls, dark teal-green grass surface, two concentric glowing ground rings one cyan one red, yellow diagonal glowing line across arena floor, small 3D heroes clashing center, floating damage numbers, purple circular ability buttons bottom right, health percentage HUD top corners, circular timer top center. Camera dips to catch the screen then pulls back to two-shot. Interviewer leans in, impressed. Lo-fi beat steady, soft. Natural daylight on both faces. SCENE 4 — 3s. Camera pulls back wider — both in frame, loose handheld. Interviewer tilts head, slow clear delivery: \\"How long's a match?\\" Guest, instant, no pause, slow clear delivery: \\"Three minutes.\\" Interviewer, quiet disbelief: \\"Three minutes?\\" Guest nods once, slight grin: \\"Three minutes.\\" Beat drops to silence. Camera holds the wide two-shot. Single soft chord resolves. Traffic passes behind them. NEGATIVE PROMPT: news microphone, foam microphone, branded microphone, portrait phone, vertical phone, back of phone visible, phone screen blank, colosseum, Roman arena, tall arena walls, giant heroes, side-view arena, theatrical reactions, robotic delivery, studio lighting, text overlays, finger gestures, pointing, slow motion, blurry faces, distorted hands, crowd reacting to interview, music before Scene 2, artificial pauses, staged positioning, phone raised to eye level, revealing clothing", "suggestedDuration": 12, "refHint": "Attach a landscape gameplay screenshot in Extra Refs — Seedance uses it for the phone screen in Scene 3."}`
    : `{"script": "SCENE 1 — 3s. Park bench near food cart, overcast afternoon. Handheld camera bouncing — interviewer walking fast toward a woman sitting on the bench with earbuds in, scrolling. Street noise, pigeons, distant traffic. Western male interviewer, 25, beige flannel shirt, jeans, no mic. Western female guest, 23, denim jacket, canvas tote beside her — pulls earbud out as interviewer arrives. Interviewer, slightly breathless, slow clear delivery: \\"Quick — what's on your phone right now?\\" Guest looks up, half-laugh. No music — ambient only. Cool overcast grade. SCENE 2 — 5s. Same bench, loose handheld two-shot. Guest grins, slow clear delivery: \\"Blitz of Battle — I'm mid-match.\\" Flips phone to landscape, two-handed grip, screen out at chest level. Isometric top-down mobile MOBA view — circular arena with low curved concrete barrier walls, dark teal-green grass surface, two concentric glowing ground rings one cyan one red, yellow diagonal glowing line across arena floor, small 3D heroes clashing center, floating damage numbers, purple circular ability buttons bottom right, health percentage HUD top corners, circular timer top center. Lo-fi beat fades in soft. Camera catches screen then pulls to two-shot. Interviewer leans forward, impressed, slow clear delivery: \\"How long's a match?\\" Soft natural light on both faces. SCENE 3 — 3s. Camera pulls back wider. Guest, instant, no pause, slow clear delivery: \\"Three minutes.\\" Interviewer, quiet: \\"Three minutes?\\" Guest nods, slight smile: \\"Three minutes.\\" Beat drops to silence. Single soft chord. Camera holds wide. Bus passes behind. NEGATIVE PROMPT: news microphone, foam microphone, branded microphone, portrait phone, vertical phone, back of phone visible, phone screen blank, colosseum, Roman arena, tall arena walls, giant heroes, side-view arena, theatrical reactions, robotic delivery, studio lighting, text overlays, finger gestures, pointing, slow motion, blurry faces, distorted hands, crowd reacting to interview, music before Scene 2, artificial pauses, staged positioning, phone raised to eye level, revealing clothing", "suggestedDuration": 11, "refHint": "Attach a landscape gameplay screenshot in Extra Refs — Seedance uses it for the phone screen in Scene 2."}`;

  const userPrompt = `${hookLine}
${locationLine}

DURATION: ${dur}s total (${sceneCount} scenes).

GAME CONTEXT:
${gameContext}

EXAMPLE — follow this structure and tone exactly:
${exampleScript}

JSON only. SCENE labels. Include refHint (phone always appears). Every spoken line must say "slow clear delivery".`;

  console.log(`[Claude] Generating street interview script | hook="${hook?.slice(0, 60) || '(auto)'}" | location="${location || 'auto'}" | dur=${dur}s`);
  ...
  const script = (parsed.script || '').trim();
  if (!script) throw new Error('Claude returned empty script');
  const suggestedDuration = Math.max(10, Math.min(15, parseInt(parsed.suggestedDuration) || dur));
  const refHint = (parsed.refHint || '').trim() || null;
  return { script, suggestedDuration, refHint };
}
