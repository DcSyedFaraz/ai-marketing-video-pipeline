    ...
    try {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // Check for task-level errors in the top-level errors array and resolve them
          const topErrors = json?.errors;
          if (Array.isArray(topErrors) && topErrors.length > 0) {
            for (const apiErr of topErrors) {
              const errUUID = apiErr?.taskUUID;
              if (!errUUID) continue;
              const entry = this._tasks.get(errUUID);
              if (!entry) continue;
              console.error(`[${this._label}] ${entry.label} | terminal error (HTTP ${res.status}): ${apiErr.message || JSON.stringify(apiErr)}`);
              this._tasks.delete(errUUID);
              entry.onError(new Error(apiErr.message || JSON.stringify(apiErr))).catch(e =>
                console.error(`[${this._label}] onError threw:`, e?.message)
              );
            }
          }
