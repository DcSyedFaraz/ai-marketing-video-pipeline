 ...
 */
export async function planScenes(storyText, durationRange, gameContext, voiceDesc, heroDesc, images = {}, allowedDurations = []) {
...
- SEQUENTIAL FRAME PIPELINE: Scene 1 uses frame A → frame B. Scenes 2+ use extracted last frame → this scene's image. Last scene uses extracted frame → CTA image.
...
- The SUM of all scene durations MUST be between ${durMin} and ${durMax} seconds.
- Prefer shorter scenes with fast cuts for maximum engagement. Use the shortest duration (${shortestDur}s) for CTA.
- You decide how many scenes to create — pick the count that makes the video most engaging while staying within the ${durMin}–${durMax}s range.
...
Return ONLY a valid JSON object (no markdown, no code fences, no explanation):
{
  ...
  "scenes": [
    ...
    {
      "sceneNumber": 2,
      "imagePrompt": "end frame — what scene 2 video ends on...",
...
IMPORTANT: You MUST create at least 2 scenes. The total of all durations MUST be between ${durMin} and ${durMax} seconds.`;
  ...
  console.log(`[Claude] Planning scenes (${durMin}-${durMax}s) with claude-sonnet-4-6... (voice: ${voiceDesc ? 'user-provided' : 'auto-generate'}, heroImg: ${hasHeroImage}, bgImg: ${hasBgImage})`);
