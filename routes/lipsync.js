...
router.post('/api/generate-lipsync', upload.fields([
  ...
  (async () => {
    ...
    try {
      ...
      console.log(`[LipSync] Submitting task ${taskUUID} via global connection (skipResponse)...`);
      await globalPoller.getConnection().videoInference({ ...requestPayload, includeCost: true, skipResponse: true });
      console.log(`[LipSync] Task submitted OK. Registered with global poller.`);

    } catch (submitErr) {
      const errMsg = submitErr?.message || String(submitErr);
      console.error(`[LipSync] ❌ Submit failed: ${errMsg}`);
      updateHistoryEntry(taskUUID, { status: 'failed', error: errMsg, completedAt: new Date().toISOString() });
      sseEmitter.emit('task-complete', { taskUUID, type: 'lipsync', status: 'failed', error: errMsg });
    ...
    globalPoller.register(taskUUID, {
      ...
      onComplete: async (result) => {
        ...
        try {
          ...
          const ctaImages = await globalPoller.getConnection().imageInference({
            taskUUID: randomUUID(),
            model: 'google:4@3',
